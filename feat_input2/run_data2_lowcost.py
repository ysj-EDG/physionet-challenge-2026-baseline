...
DATASET = "data2 timegrid_v2"
CACHE_ROOT = ROOT / "npz_data2"
...
OUT = ROOT / "feat_input2/output/data2_lowcost"
QC_OUT = OUT / "qc"
SITE_OUT = OUT / "site_audit"
LOSO_OUT = OUT / "loso_lr"
SUMMARY_CACHE = OUT / "cache/data2_patient_summaries.npz"
...
def full_scan(records, rules, rules_by_key):
    ...
    for ordinal, row in enumerate(records, 1):
        ...
        if ordinal % 250 == 0:
            print(f"SCAN1 {ordinal}/6600", flush=True)
...
def build_p3_matrices(records, fold_parameters, rules_by_key):
    ...
    for ordinal, row in enumerate(records, 1):
        ...
        if ordinal % 250 == 0:
            print(f"SCAN2 {ordinal}/6600", flush=True)
