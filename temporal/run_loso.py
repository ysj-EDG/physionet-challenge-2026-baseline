...
def sampler_loader(records,indices,state,shuffle,batch_size=64):
    ...
    loader=DataLoader(BlockDataset(records,indices,state,shuffle),batch_size=batch_size,sampler=sampler,collate_fn=collate,num_workers=0,generator=generator)
    return loader,cells

def natural_loader(records,indices,state,shuffle,batch_size=64):
    return DataLoader(BlockDataset(records,indices,state,shuffle),batch_size=batch_size,shuffle=False,collate_fn=collate,num_workers=0)
...
def run_tcn_fold(records,inner_train,inner_val,holdout,fold_dir,modality,shuffle,device,max_epochs=30,min_epochs=8,patience=5,batch_size=64):
    ...
    for epoch in range(1,max_epochs+1):
        ...
        history.append(row); print(json.dumps({'arm':'S' if shuffle else 'R','epoch':epoch,'selection':selection,'loss':row['train_loss']}),flush=True)
        if epoch>=min_epochs and stale>=patience: break
    if best_state is None: raise RuntimeError('No checkpoint selected')
    model.load_state_dict(best_state); torch.save({'state_dict':best_state,'best_epoch':best_epoch,'selection_score':best},fold_dir/'best_checkpoint.pt')
    _,yt,score,ages=collect(model,natural_loader(records,holdout,state,shuffle,batch_size),records,device); metrics=metric_bundle(yt,score,ages); metrics.update({'arm':'S' if shuffle else 'R','best_epoch':best_epoch,'validation_site_macro_ac':best,'training_wall_time_sec':time.time()-started,'initial_weight_hash':initial_hash})
...
def run(modality,output_root,data_root=DATA_ROOT,smoke=False,only_holdout=None):
    ...
    for hold in holds:
        ...
        for arm in ('O','S','R'):
            ...
            else: metric=run_tcn_fold(records,inner_train,inner_val,holdout,fold,modality,arm=='S',device,max_epochs=2 if smoke else 30,min_epochs=2 if smoke else 8,patience=1 if smoke else 5)
    ...
    json_dump(output_root/'config.json',{'source_git_sha':git_head(),'dataset':'DATA2','data_root':str(Path(data_root).resolve()),'modality':modality,'records_total':6600,'records_usable':len(records),'unusable_no_complete_block':unusable,'load_wall_time_sec':load_sec,'device':str(device),'smoke':smoke,'seed':7})
...
if __name__=='__main__':
    p=argparse.ArgumentParser(); p.add_argument('--modality',choices=MODALITY_DIM); p.add_argument('--data-root',default=str(DATA_ROOT)); p.add_argument('--output-root'); p.add_argument('--smoke',action='store_true'); p.add_argument('--only-holdout',choices=SITES)
    a=p.parse_args()
    if a.smoke: smoke(a.data_root,a.output_root or 'temporal_results/smoke')
        ...
        run(a.modality,a.output_root,a.data_root,False,a.only_holdout)
